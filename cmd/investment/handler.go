...
func (w *Worker) RequestWithdrawal(ctx context.Context, invID uuid.UUID, amount int64, noticeMontrhs int32) error {
	...
	withdrawal := database.InvestmentWithdrawal{
		InvestmentID:       inv.ID,
		Amount:             amount,
...
func (w *Worker) ProcessEligibleWithdrawals(ctx context.Context) error {
	...
	for _, wdr := range withdrawals {
		...
		err = w.ledger.Transfer(ctx, w.capitalAccID, wdr.InvestmentID, wdr.Amount, fmt.Sprintf("Investment withdrawal ID %v", wdr.ID))
		if err != nil {
...
func StartScheduler(ctx context.Context, worker *Worker, accrualWorker *AccrualWorker) error {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				return fmt.Errorf("Error processing eligible withdrawals: %v", err)
			}
			err = accrualWorker.ProcessDueAccruals(ctx)
			if err != nil {
				return fmt.Errorf("Error processing due accruals: %v", err)
			}
		case <-ctx.Done():
			fmt.Println("Scheduler stopping...")
			return nil
